...
cp /root/gdbserver/$version/gdb-$version.tar.xz ~/ && tar xJf "gdb-$1.tar.xz" && cd "gdb-$1"
...
CC="arm-linux-gnueabi-gcc" CXX="arm-linux-gnueabi-gcc" ../gdb/gdbserver/configure --target=arm-linux-gnueabi --host="x86_64-linux-gnu"
make LDFLAGS=-static -j $nproc && cp gdbserver ~/gdbserver/$version/arm-gdbserver && cd ..
